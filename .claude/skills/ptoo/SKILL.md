...
2. Leia as funções-fonte relevantes e referencie `file:line`:
   `normalize_illumination` ([photo_to_outline.py:549](../../../photo_to_outline.py)),
   `segment_tool` (:580), `symmetrize_mask` (:684), `regularize_silhouette` (:721, base do
   `--mask-smooth-mm`), `extract_outline` (:739), `enforce_min_radius` (:374),
   `fit_closed_beziers_anchored` (:1209) e a lógica de protuberância (`PROTRUSION_DEV_MM`, :109).

...
- **Rotate** / **Pan** — giro (0.1°) e deslocamento lateral (0.1 mm) finos de foto+contorno.
  **Persistem** (junto com os **pins** — nós reposicionados): ao Finalize o acumulado vai p/
  `images/<name>.adjust.json` e é reaplicado em toda execução (os pins deformam a silhueta
  detectada p/ passar por eles); o status bar mostra sempre `rot ±x.xx°` / `pan ±x.xx mm` e
  `pins N`.
